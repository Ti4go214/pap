<?php
/**
 * @file migration_entidades.php
 * @brief Correção da estrutura das tabelas de clientes e fornecedores
 * @author Antigravity
 * @date 2026-04-28
 */
...
// Proteção básica - apenas admin pode executar
if (!isset($_SESSION['user']) || ($_SESSION['role'] ?? 1) != 0) {
    die("Acesso negado. Apenas administradores podem executar migrações.");
}

echo "<h2>Migração de Estrutura de Entidades</h2>";

// 1. Corrigir tabela clientes
echo "<h3>1. Corrigindo tabela clientes...</h3>";

// Verificar se a tabela existe e tem a estrutura correta
$check_clientes = $conn->query("DESCRIBE clientes");
if ($check_clientes) {
    echo "<p>Tabela clientes existe. A verificar estrutura...</p>";
    
    // Eliminar tabela existente para recriar com estrutura correta
    $conn->query("DROP TABLE IF EXISTS clientes");
    echo "<p>Tabela clientes removida.</p>";
}

// Criar tabela clientes com estrutura correta
...
$check_fornecedores = $conn->query("DESCRIBE fornecedores");
if ($check_fornecedores) {
    echo "<p>Tabela fornecedores existe. A verificar estrutura...</p>";
    
    // Eliminar tabela existente para recriar com estrutura correta
    $conn->query("DROP TABLE IF EXISTS fornecedores");
    echo "<p>Tabela fornecedores removida.</p>";
}

// Criar tabela fornecedores com estrutura correta
...
echo "<p><a href='../gestao.php'>Voltar à Gestão</a></p>";
?>
